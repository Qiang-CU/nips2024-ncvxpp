...
class Simulation:
    def __init__(self, eps, batch, file_loc, num_epoch, a0, a1, log_maxiter, device):
        ...
        self.loss_fn = BCELossWithL2(self.model, lambda_reg=1e-3)
    ...
    def shift(self, x, approx, eps): #TODO
        # x = x - eps * approx #TODO: 这里是随便写的先
        return x
    ...
    def test(self, eps):
        ...
        XX =torch.zeros_like(self.test_X, requires_grad=True)
        output = self.model(XX)
        output.backward(torch.ones_like(output))
        X_test_shift = self.shift(self.test_X, XX.grad, eps).to(self.device)
    ...
    def train(self, epoch):
        ...
        for idx, (X, Y) in enumerate(self.train_loader):
            ...
            output = self.model(XX)
            output.backward(torch.ones_like(output))
            X_shift = self.shift(X, XX.grad, self.eps).to(self.device)
            ...
            if self.iter in self.sample_time:
                ...
                output = self.model(XX)
                output.backward(torch.ones_like(output)) # TODO: 为什么不加torch.ones_like(output)就不行?
                X_full_shift = self.shift(self.train_X, XX.grad, eps).to(self.device)

                # get loss and acc based on shifted data
                self.optimizer.zero_grad()
                pred = self.model(X_full_shift)
                total_loss = self.loss_fn(pred, self.train_Y.to(self.device))
                acc = self.compute_acc(pred, self.train_Y)
                total_loss.backward()
